...
int main() {
    ...
    if (note) {
        if (note->right || note->left) {
            ...
            if (note->right) {
                if (note->right->right) {
                    cout << note->right->right->inf << " ";
                    flag = true;
                }
                if (note->right->left) {
                    cout << note->right->left->inf << " ";
                    flag = true;
                }
            }
            if (note->left) {
                if (note->left->right) {
                    cout << note->left->right->inf << " ";
                    flag = true;
                }
                if (note->left->left) {
                    cout << note->left->left->inf << " ";
                    flag = true;
                }
            }
            if (!flag) {
                cout << "У узла нет 'внуков'" << endl;
            }
        }
        else {
            cout << "У узла нет 'детей'" << endl;
        }

    }
    else
        cout << "Такого узла нет в этом дереве\n";
    return 0;
}